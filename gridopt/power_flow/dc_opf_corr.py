...
class DCOPF_Corr(PFmethod):
    ...
    parameters = {'quiet' : False,
                  'thermal_limits': True,
                  'thermal_factor': 1.,
                  'max_ramping': 0.1,     # fraction of max output
                  'inf_flow': 1e4}
    
    def __init__(self):
        
        PFmethod.__init__(self)
        self.parameters = DCOPF_Corr.parameters.copy()
    ...
    def solve(self,net,contingencies):
        ...
        max_ramping = params['max_ramping']
        ...
        GWJ_list = [(G,W,J)]
        u_list = [up,uw,uz]               # p,w,z
        l_list = [lp,lw,lz]               # p,w,z
        ...
        for cont in contingencies:
            ...
            u_list += [max_ramping*np.abs(up),up,uw,uz]  # dp,q,w,z
            l_list += [-max_ramping*np.abs(up),lp,lw,lz] # dp,q,w,z
            b_list += [b,np.zeros(J.shape[0]),zp]
        ...
        for i in range(num_blocks):
            ...
            row1 = (4*num_blocks-1)*[None]
            row1[4*i] = G
            row1[4*i+1] = -W
            A.append(row1)

            row2 = (4*num_blocks-1)*[None]
            row2[4*i+1] = J
            row2[4*i+2] = -eye(J.shape[0],format='coo')
            A.append(row2)
            
            if i > 0:
                row3 = (4*num_blocks-1)*[None]
                row3[0] = Ip
                row3[4*i-1] = Ip
                row3[4*i] = -Ip
                A.append(row3)
