...
class MS_DCOPF_GR(MS_DCOPF_Method):
    ...
    def solve(self,net,forecast):
        ...
        def apply(cls,t,x_prev,Wt):
            ...
            x_list,Q_list,gQ_list = self.problem.eval_stage_approx(t,[Wt[-1]],x_prev,quiet=True,tf=t)
            assert(len(x_list) == 1)
            x = x_list[0]
            p,q,w,s,y,z = self.problem.separate_x(x)
            p_prev = x_prev[:self.problem.num_p]
            
            # Check dims
            assert(p.shape,(self.problem.num_p,))
            assert(p_prev.shape,(self.problem.num_p,))
